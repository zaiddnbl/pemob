...
class _AdminLaporanScreenState extends State<AdminLaporanScreen>
  ...
  List<PembayaranModel> _semuaPembayaran = [];
  ...
  Future<void> _loadData() async {
    ...
    final data = await FirestoreService.getSemuaPembayaran();
    if (mounted) {
      setState(() {
        _semuaPembayaran = data;
        _isLoading = false;
      });
    }
  }

  List<PembayaranModel> get _filteredData {
    if (_selectedBulan == 'Semua') return _semuaPembayaran;
    final bulanIdx = _bulanList.indexOf(_selectedBulan);
    return _semuaPembayaran.where((p) {
      try {
        final m = int.parse(p.tanggal.split('-')[1]);
        return m == bulanIdx;
      } catch (_) {
        return false;
      }
  ...
  // Tampilan per user: grouping by noKios
  Map<String, Map<String, dynamic>> get _groupedByUser {
    final map = <String, Map<String, dynamic>>{};
    for (final p in _filteredData) {
      ...
      map[p.noKios]!['total'] =
          (map[p.noKios]!['total'] as double) + p.jumlah;
  ...
  Future<void> _exportExcel(bool perUser) async {
    ...
    if (perUser) {
      ...
      for (final entry in _groupedByUser.entries) {
        final d = entry.value;
        sheet.appendRow([
          TextCellValue(d['namaPedagang']),
          TextCellValue(d['noKios']),
          TextCellValue('${d['jumlahTransaksi']}x'),
          TextCellValue(_formatRupiah(d['total'])),
        ]);
      }
    } else {
      sheet.appendRow([
          ...
          if (perUser) {
            return [
              ...
              pw.Table.fromTextArray(
                ...
                data: _groupedByUser.values.map((d) => [
                  d['namaPedagang'],
                  d['noKios'],
                  '${d['jumlahTransaksi']}x',
            ...
            child: TabBarView(
              ...
              children: [
                ...
                _groupedByUser.isEmpty
                    ...
                    : ListView.builder(
                  padding: const EdgeInsets.all(12),
                  itemCount: _groupedByUser.length,
                  itemBuilder: (context, index) {
                    final entry = _groupedByUser.entries
                        .elementAt(index);
                    final d = entry.value;
                    return Container(
                      margin: const EdgeInsets.only(bottom: 8),
                              ...
                              children: [
                                ...
                                Text('Kios ${d['noKios']}',
                                    style: const TextStyle(
                                        ...
                                        AppTheme.greyText)),
                                Text(
                                    '${d['jumlahTransaksi']} transaksi di bulan ini',
                                    style: const TextStyle(
                                        fontSize: 11,
                                        color:
                                        AppTheme.greyText)),
